...
const Timer = () => {
  const { LoginButton, setHeader, isMenuOpen, setIsMenuOpen } = useData();
  ...
  const handleMemoCreate = () => {
    if (isMemoConfirmed) {
      //만약 true면 완료가 되어 있는 상태(수정 버튼이 올라와있는 상태)
      //아무 동작 안함(버튼만 바뀜)
    } else {
      //false일 경우, 즉 수정 버튼 상태에서 눌렀을 경우
      memoCreateApi();
    }
      ...
      {!isMenuOpen && (
        <S.TimerPageWrapper>
          <S.TimerWrapper>
            ...
            <S.MyStatusWrapper>
              <S.MyStatusTimer>{`${formatWorkTime(
                workTime
              )}째 작업 중`}</S.MyStatusTimer>
              <S.MyStatusGraphic $profile={isLogin ? userProfile : "gentle"} />
            </S.MyStatusWrapper>
            <S.TimerButtonWrapper>
              <S.TimerButton
                onClick={handleTimerStartOrReset}
                $redMode={isTimerStart}
              >
                {isTimerStart ? "작업 타이머 중지" : "작업 타이머 시작"}
              </S.TimerButton>
              <S.TimerButton
                onClick={handleTimerStopToggle}
                $redMode={isBreakTime}
              ...
              <S.TimerSubTitle>
                휴식 버튼으로 작업 타이머를 중지하지 않고 휴식만 할 수 있어요
              </S.TimerSubTitle>
            </S.TimerButtonWrapper>
          </S.TimerWrapper>
                ...
                <S.MyCardInputWrapper>
                  ...
                  <S.MyCardInputContainer>
                    <S.MyCardTextArea
                      ...
                      disabled={isMemoConfirmed}
                      value={content}
                      placeholder={"상태메시지를 입력하세요."}
                    />
                    <S.MyCardInputButton onClick={handleMemoCreate}>
                      {isMemoConfirmed ? "수정" : "완료"}
                    </S.MyCardInputButton>
                  </S.MyCardInputContainer>
                </S.MyCardInputWrapper>
              </S.MyOnlineCard>
                ...
                {userMemoDatas?.map((obj, idx) => (
                  ...
                  <S.OtherCardWrapper key={`otherMember_${idx}`}>
                    ...
                    <S.OtherCardTextDivBox>
                      <S.OtherCardTextDiv>{obj.content}</S.OtherCardTextDiv>
